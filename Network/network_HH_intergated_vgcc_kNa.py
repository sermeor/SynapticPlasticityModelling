...
@njit
def I_Na(m, h, Vm, c_in, c_out):
  g_Na = 120.0  # maximum sodium conductance (mS/cm^2)
  E_Na = equilibrium_potential(c_in, c_out) #50.0 sodium reversal potential (mV)
  return g_Na * (m**3) * h * (Vm - E_Na)


##Function of potassium channels current (mA/cm^2).
@njit
def I_K(n, Vm, c_in, c_out):
  E_K = equilibrium_potential(c_in, c_out) #-77.0 potassium reversal potential (mV)
...
def I_L(Vm):
  ...
  P_k = 1  # relative potassium permeability
  P_na = 0.3  # reltive sodium permeability
  i_lk = g_L * P_k * (Vm - E_K)
  i_lna = g_L * P_na * (Vm - E_Na)
...
@njit
def equilibrium_potential(c_in, c_out):
  R = 8.31  #J*K^-1*mol^-1
  T = 308.15  #K
  F = 96485  #C*mol^-1

  return R * T / F * np.log(c_out / c_in)

#pump current maintaining homeostasis
@njit
def I_pump(in_Na):
  I_maxp = 40  #uA/cm^2
  k_Na = 0.1  #mM^-1
  ...
  for i, c_in in enumerate(in_Na):
    if c_in > Na_s:
       I_p[i] = I_maxp / (1 + np.exp(k_Na * (c_in - Na_s)))
  return I_p

@njit
def delta_intracellular(pump, leak, current, species_factor):
  p = 4000  #cm^-1
  F = 96485  #C*mol^-1

  return p / F * (species_factor * pump - current - leak)

@njit
def delta_extracellular(delta_int):
  vol_ratio = 0.2

  return -delta_int * vol_ratio
...
##Function of fraction of TrkB receptor BDNF-bound(sigmoid).
...
### TESTING ----------------------------------
def synaptogenesis_weights(camkII, trkb, p75_ntr, actvt):
  #recent activity frequency changes sign of CaMKII contribution
  factor = np.ones(N, dtype=np.float32)
  factor[np.where(actvt < 0.5)[0]] = -1
  p75_ntr = p75_ntr * factor

  return 0  #sigmoid(camkII + trkb - p75_ntr)
...
def comp_model(t, y, N, NE):
  ...
  e5ht = 0  #extracellular serotonin.

  spike = spike_boolean(y[0])  #Discrete spikes.
  CaMKII_bound = CaMKII(y[5])  #Bound Ca2+ to CaMKII protein.
  trkB_bound = TrkB(y[10])  #Bound BDNF to trkB.
  p75NTR_bound = p75_NTR(y[10])  #Bound proBDNF and BDNF to p75NTR.
  ...
  #Conductances.
  g_AMPA = g_AMPA_calc(a1, y[15:], y[7], N, NE)  #Conductance factor of AMPA channels.
  g_GABA_A = g_GABA_A_calc(a2, y[15:], y[7], N, NE)  # Conductance factor of GABA A channels.
  g_NMDA = g_NMDA_calc(a3, y[15:], y[8], N, NE)  # Conductance factor of NMDA channels.
  g_GABA_B = g_GABA_B_calc(a4, y[15:], y[8], N, NE)  # Conductance factor of GABA B channels.
  ...
  #y[4] = c, activation of gatting variable c.
  ...
  #y[9] = intracellular sodium
  #y[10] = intracellular potassium
  #y[11] = extracellular sodium
  #y[12] = extracellular potassium

  #y[13] = Activity state of neuron (low/high state).
  #y[14] = BDNF factor. [0,1]
  #y[15:] = connectivity matrix.

  i_na = I_Na(y[1], y[2], y[0], y[9], y[11])
  i_k = I_K(y[3], y[0], y[10], y[12])
  i_lk, i_lna = I_L(y[0])
  i_leak = i_lk + i_lna
  i_pump = I_pump(y[9])

  #Differential equations.
  dy[0] = (-i_na - i_k - i_leak - i_pump - I_VGCC(y[4], y[0]) - I_AMPA(g_AMPA, y[0]) - I_NMDA(g_NMDA, y[0]) - I_GABA_A(g_GABA_A, y[0]) - I_GABA_B(g_GABA_B, y[0]) + I_noise_e - I_noise_i) / C_m
  ...
  dy[9] = delta_intracellular(i_pump, i_lna, i_na, -3)
  dy[10] = delta_intracellular(i_pump, i_lk, i_k, 2)
  dy[11] = delta_extracellular(dy[9])
  dy[12] = delta_extracellular(dy[10])

  dy[13] = a15 * spike * (1 - y[13]) - a16 * y[13]

  ##TESTING--------------------------------
  a17 = 0.1  #Rate of increase of BDNF depending on neuron activity (ms-1).
  a18 = 0.1  #Rate of increase of BDNF when bAP event happens (ms-1).
  a19 = 0.1  #Rate of increase of BDNF...
  a20 = 0.1
  a21 = 0.1

  dy[14] = 0  #w_act*(act - sigma) + w_bAP * bAP + w_Neurot * (Neurot - Neurot0) + w_ ....
  ##--------------------------------

  dy[15:] = connectivity_update(y[15:], y[13], N, NE)
...
Cai_0 = 0.1 * np.ones(N, dtype=np.float32)  #Internal calcium (uM).
N_0 = np.zeros(N, dtype=np.float32)  #Neurotransmitter.
g_fast_0 = np.zeros(N, dtype=np.float32)  #Fast conductances.
g_slow_0 = np.zeros(N)  #Slow conductances.

Nai_0 = 15000*np.ones(N, dtype=np.float32) #Na internal concentration (uM). 
Ki_0 = 145000*np.ones(N, dtype=np.float32) #K internal concentration (uM). 
Nae_0 = 145000*np.ones(N, dtype=np.float32) #Na external concentration (uM). 
Ke_0 = 4500*np.ones(N, dtype=np.float32) #K external concentration (uM).

act = 0.5 * np.ones(N, dtype=np.float32)  #Activity state.
BDNF_0 = 0.5 * np.ones(N, dtype=np.float32)  #BDNF levels.
C = np.random.rand(N, N).flatten().astype(np.float32)  #Connectivity matrix.

y0 = np.concatenate((Vm, m, h, n, c, Cai_0, N_0, g_fast_0, g_slow_0, Nai_0, Ki_0, Nae_0, Ke_0, act, BDNF_0, C), axis=0)  #Flatten initial conditions.
...
#Get results
y = [
  sol.y[:N, :], sol.y[N:2 * N, :], sol.y[2 * N:3 * N, :],
  sol.y[3 * N:4 * N, :], sol.y[4 * N:5 * N, :], sol.y[5 * N:6 * N, :],
  sol.y[6 * N:7 * N, :], sol.y[7 * N:8 * N, :], sol.y[8 * N:9 * N, :],
  sol.y[9 * N:10 * N, :], sol.y[10 * N:11 * N, :], sol.y[11 * N:12 * N, :],
  sol.y[12 * N:13 * N, :], sol.y[13 * N:14 * N, :], sol.y[14 * N:15 * N, :],
  sol.y[15 * N:, :].reshape(N, N, sol.t.shape[0])
]
...
plt.figure()
plt.subplot(5, 1, 1)
plt.plot(y[0][n_neuron, :])

plt.subplot(5, 1, 2)
plt.scatter(range(0, sample_span), spike_matrix[n_neuron])

plt.subplot(5, 1, 3)
plt.plot(y[6][n_neuron, :])

plt.subplot(5, 1, 4)
plt.plot(y[7][n_neuron, :])
plt.plot(y[8][n_neuron, :])

plt.subplot(5, 1, 5)
plt.plot(y[5][n_neuron, :])
plt.show()