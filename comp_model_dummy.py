...
def comp_model_dummy(t, y, v2, ssri_molecular_weight, SSRI_start_time, SSRI_repeat_time, SSRI_q_inj, 
  ...
  dy[0] = 0#TRPin - VTRPin(y[0]) - a1 * (y[0] - btrp0)
  dy[1] = 0#inhibsyn5HTto5HT(y[11], gstar_5ht_basal) * VTPH(y[3], y[2]) - VDRR(y[1], NADPH, y[2], NADP)
  dy[2] = 0#VDRR(y[1], NADPH, y[2], NADP) - inhibsyn5HTto5HT(y[11], gstar_5ht_basal) * VTPH(y[3], y[2]) 
  dy[3] = 0#VTRPin(y[0]) - inhibsyn5HTto5HT(y[11], gstar_5ht_basal) * VTPH(y[3], y[2]) - VPOOL(y[3], y[10]) - a12 * y[3]
  dy[4] = 0#inhibsyn5HTto5HT(y[11], gstar_5ht_basal) * VTPH(y[3], y[2]) - VAADC(y[4])
  dy[5] = 0#VAADC(y[4]) - VMAT(y[5], y[6]) - VMAT(y[5], y[7]) + VSERT(y[8], y[19], ssri, allo_ssri_ki(ssri)) - TCcatab(y[5]) - a15 * (y[5] - y[8])
  dy[6] = 0#VMAT(y[5], y[6]) - a16 * fireht(t, inhibR5HTto5HT(y[11], gstar_5ht_basal) * inhibRHAto5HT(y[15], gstar_ha_basal)) * y[6] + vht_trafficking(y[6], vht_basal)
  dy[7] = 0#VMAT(y[5], y[7]) - a15 * vht_trafficking(y[6], vht_basal)
  dy[8] = 0#a16 * fireht(t, inhibR5HTto5HT(y[11], gstar_5ht_basal) * inhibRHAto5HT(y[15], gstar_ha_basal)) * y[6] - VSERT(y[8], y[19], ssri, allo_ssri_ki(ssri)) - a11 * H1ht(y[8], eht_basal) * VUP2(y[8]) - a14 * y[8] + a8 * (y[5] - y[8]) + a9 * (y[14] - y[8]) 
  dy[9] = 0#TCcatab(y[5]) + TCcatab(y[14]) - a10 * y[9] 
  dy[10] = 0#VPOOL(y[3], y[10]) - a13 * y[10]
  dy[11] = 0#a2 * y[13]**2 * (g0 - y[11]) - a3 * y[12] * y[11]  
  dy[12] = 0#a4 * y[11]**2 * (t0 - y[12]) - a5 * y[12]
  dy[13] = 0#a6 * y[8] * (b0 - y[13]) - a7 * y[13] 
  dy[14] = 0#a11 * H1ht(y[8], eht_basal) * VUP2(y[8]) - TCcatab(y[14]) - a9 * (y[14] - y[8])  
  dy[15] = 0#a17 * y[17]**2 * (gh0 - y[15]) - a18 * y[16] * y[15]
  dy[16] = 0#(a19*y[15]**2 * (th0 - y[16])  - a20 * y[16]) 
  dy[17] = 0#(a21*y[29]*(bh0 - y[17])  - a22*y[17]) 
  dy[18] = 0#k_ps * y[20] - k_sp * y[18]
  dy[19] = 0#dy[18] - k_si * y[19] + k_is * y[21]
  dy[20] = 0#k_sp * y[18]  - k_ps * y[20]
  dy[21] = 0#k_si * y[19]  - k_is * y[21]
  ...
  dy[38] = 0#(b21*y[37]**2*(t05ht - y[38])  - b22*y[38])
  dy[39] = 0#(b23*y[8]*(b05ht - y[39])  - b24*y[39])
  dy[40] = 0#VHTLg(y[31]) - (1 + b12*mc_activation(t, mc_switch, mc_start_time)) * y[53] * VHTDCg(y[40]) - b9 * y[40] + b10 * y[41]
  dy[41] = 0#b9 * y[40] - b10 * y[41] - b11 * y[41]
